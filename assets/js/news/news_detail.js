...
const saveContent = (isToast = false) => {
    ...
    $.ajax({
        ...
        url: `${baseUrl}/NewsAPI/createNews`,
        data: params,
        processData: false,
        contentType: false
    }).done(function (data) {
        if (data.code == 400) {
            toastr.error(`${data.message ? data.message : "Error while create news"} Code: 400`);
            return;
        }
        if (isToast) {
...
$(function () {
    refreshContent("eng");
    $('#news_content_eng').summernote()
    $('#news_content_ita').summernote()
